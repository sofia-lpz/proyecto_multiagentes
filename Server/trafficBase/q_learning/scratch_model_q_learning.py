...
class CityModel(Model):
    def __init__(self):
        ...
        self.datacollector = DataCollector(
            model_reporters={
                "Active Cars": lambda m: len([a for a in m.schedule.agents if isinstance(a, Q_Car)]),
                "Completed Cars": lambda m: m.cars_completed,
                "Average Completed Cars": lambda m: m.cars_completed / m.total_steps
            }
        )
    ...
    def spawn_car(self): 
        ...
        car0 = Q_Car(self.car_count, self, destination)
        self.grid.place_agent(car0, (0, 0))
        self.schedule.add(car0)
        self.car_count += 1

    def spawn_cars_per_step(self, steps):
        cars_spawned = False

        if self.schedule.steps % steps == 0 and self.destinations:
            corners = [
                (0, 0),                    # Bottom left
            ...
            for corner in corners:
                ...
                if not any(isinstance(content, Q_Car) for content in cell_contents):
                    destination = self.random.choice(self.destinations)
                    Q_Car = Q_Car(f"car_{self.car_count}", self, destination)
                    self.grid.place_agent(Q_Car, corner)
                    self.schedule.add(Q_Car)
                    self.car_count += 1
                    cars_spawned = True
